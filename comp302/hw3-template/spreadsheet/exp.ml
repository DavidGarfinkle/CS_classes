...
#use "sources.ml";;
  ...
  (* ————————————————————–—————————————————————————————————————————————– *)
  let add x y =
      let result a = match a with 
          | Num(z)          -> !z 
          | Plus(z, _, _)   -> !z
          | Times(z, _, _)  -> !z
      in Plus( ref (( result x ) + (result y)), x, y )
  ...
  let mult x y =
      let result a = match a with 
          ...
          | Plus(z, _, _)   -> !z
          | Times(z, _, _)  -> !z
      in Times( ref (( result x ) * (result y)), x, y )
  ...
  let rec compute_column m f = List.map f m;;
  ...
  let rec update e = 
        ...
        in match e with
            | Num(z) -> ()
            | Plus(z, l, r)   -> if update l = update r then z:=  ( result l) + (result r)
            | Times(z, l, r)  -> if update l = update r then z:=  ( result l) * (result r)
